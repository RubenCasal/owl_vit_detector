#include "owl_vit_detector/theta_driver_lib.hpp"
...
void ThetaDriver::publishImage(GstMapInfo map) {
    const int width = use4k_ ? 3840 : 1920;
    const int height = use4k_ ? 1920 : 960;
    

    auto create_image_msg = [&](const std::string& frame_id, const uint8_t* data, int w, int h) {
    ...
    image_stitched_pub_->publish(*create_image_msg(camera_frame_, map.data, width, height));
...
void ThetaDriver::onInit() {
    image_stitched_pub_ = this->create_publisher<sensor_msgs::msg::Image>("stitched_image", 1);
